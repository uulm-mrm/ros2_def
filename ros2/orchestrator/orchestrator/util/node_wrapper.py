...
class OrchestratorWrapperNode:
    def __init__(self, node: Node, topics: Union[List[str], None] = None):
        ...
        self.orchestrator_status_pub: Publisher = self.node.create_publisher(Status, "/status", 10)  # type: ignore
    ...
    def create_subscription(self, topic_type: type, topic: str, callback: Callable[[Any], None], *args: Any,
        ...
        callback = (lambda msg, topic=topic: self.handle(msg,
                                                         topic)) if self.topics is None or topic in self.topics else callback
        return self.node.create_subscription(topic_type, topic, callback, *args,  # type: ignore
                                             **kwargs)

    def destroy_subscription(self, subscription: Subscription) -> bool:
        return self.node.destroy_subscription(subscription)

    def handle(self, msg: Any, topic: str) -> None:
